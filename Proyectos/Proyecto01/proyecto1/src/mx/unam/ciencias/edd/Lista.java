...
public class Lista<T> implements Iterable<T> {
    ...
    /**
     * Metodo para representar en una imagen SVG una lista.
     * @return - Nos regresa la imagen SVG en forma de cadena.
     */
    public String generaScalableVectorGraphics() {
	int espacio, espacio1, cuadros, cuadros1, largo, longitud;
	...
	String svg = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"
	    +"<svg width=\""+longitud*largo+"\" height=\"200\">\n"
	    +"<g>\n";
	for(int i = 0; i < longitud; i++){
	    svg= svg.concat(creaCuadro((cuadros+espacio/2), 75));
	    if(i!=0)
		svg= svg.concat(creaFlecha((((cuadros1+espacio1)/2)), 100,
					   (((cuadros1+espacio1)/2)+25), 100));
	    svg= svg.concat(creaTexto((((cuadros1+espacio1)/2)+50), 110, get(i)));
	    espacio+=50;
	    cuadros+=50;
	    espacio1+=75;
	    cuadros1+=75;
    ...
    //Método privado para crear las 5 flechas del SVG.
    private String creaFlecha(int x1, int y1,
			       int x2, int y2){
	String linea = "";
	linea = linea.concat("<line x1=\""+(x1)+
			     "\" y1=\""+(y1)+"\" x2=\""+(x2)+
			     "\" y2=\""+(y2)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
	linea = linea.concat("<line x1=\""+(x1)+
			     "\" y1=\""+(y1)+"\" x2=\""+(x1+5)+
			     "\" y2=\""+(y1+5)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
	linea = linea.concat("<line x1=\""+(x1)+
			     "\" y1=\""+(y1)+"\" x2=\""+(x1+5)+
			     "\" y2=\""+(y1-5)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
	linea = linea.concat("<line x1=\""+(x2)+
			     "\" y1=\""+(y2)+"\" x2=\""+(x2-5)+
			     "\" y2=\""+(y2+5)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
	linea = linea.concat("<line x1=\""+(x2)+
			     "\" y1=\""+(y2)+"\" x2=\""+(x2-5)+
			     "\" y2=\""+(y2-5)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
	return linea;
    ...
    //Metodo privado para crear los cuadros del SVG.
    private String creaCuadro(int x, int y){
	return "<rect x=\""+(x)
	    +"\" y=\""+(75)+"\" width=\"50\" height=\"50\""
	    +" stroke=\"black\" stroke-width=\"3\" fill =\"white\" />\n";
    }
    
    //Metodo privado para crear el texto del SVG.
    private String creaTexto(int x, int y, T text){
	return "<text fill=\"black\""
	    +" font-family=\"sens-serif\" font-size=\"30\""
	    +" x=\""+(x)+"\" y=\""+(y)+"\""
