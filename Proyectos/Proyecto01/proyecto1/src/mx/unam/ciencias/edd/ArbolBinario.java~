...
public abstract class ArbolBinario<T> implements Iterable<T> {
    ...
    /**
     * Metodo que genera una cadena que representa una imagen
     * en el formato SVG.
     * @return - una cadena que forma una imagen de un arbol.
     */
	...
	String svg = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"
	    +"<svg width=\""+profundidad*largo+"\" height=\""+profundidad*180+"\">\n"
	    +"<g>\n";
	svg = svg.concat(creaVertice(this.raiz, 0, profundidad*(largo/2), profundidad*(largo/4)));
	svg = svg.concat("</g> \n </svg>");
	return svg;
    }

    /* Método privado que crea por niveles los vértices y aristas*/
    private String creaVertice(Vertice<T> vertice, int nivel, int lugar, int div){
	String svg = "";
	double radio = 20;
	Color colorV = vertice.getColor();
	String colorTexto = "black";
	String color = "white";
	if(colorV == Color.NEGRO || colorV == Color.ROJO){
	    colorTexto = "white";
	    if(nivel >= 5)
		radio = 14;
	    if(colorV == Color.NEGRO)
		color = "black";
	    else
		color = "red";
	}
	/*Primer caso, para evitar que se parta el círculo*/
	if(nivel == 0){
	    /*Creo el círculo*/
		svg= "<circle cx=\""+(lugar)
		    +"\" cy=\""+(radio+3)+"\" r=\""+(radio)+"\" stroke=\"black\" stroke-width=\"3\" fill =\""+(color)+"\" />\n";
	    /*Creo el texto*/
		svg= svg.concat("<text fill=\""+(colorTexto)+"\""
			    +" font-family=\"sens-serif\" font-size=\"15\""
			    +" x=\""+(lugar)+"\" y=\""+(25)+"\""
			    +" text-anchor=\"middle\">"+vertice.elemento+"</text>");
	    /*Caso donde tiene hijo derecho, creo linea y recurso sobre hijo derecho*/
	    if(vertice.hayDerecho()){
		svg = svg.concat("<line x1=\""+(lugar)+
				 "\" y1=\""+((nivel*150)+40)+"\" x2=\""+(lugar+div)+
				 "\" y2=\""+((nivel+1)*150)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
		svg= svg.concat(creaVertice(vertice.derecho, nivel+1, ((div)+lugar), div/2));
	    }
	    /*Caso donde tiene hijo izquierdo, creo linea y recurso sobre hijo izquierdo*/
	    if(vertice.hayIzquierdo()){
		svg = svg.concat("<line x1=\""+(lugar)+
				 "\" y1=\""+((nivel*150)+40)+"\" x2=\""+(lugar-div)+
				 "\" y2=\""+((nivel+1)*150)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
		svg= svg.concat(creaVertice(vertice.izquierdo, nivel+1, (lugar-div), div/2));
	    }
	}else{
	    /*Todos los demás casos que no son la raíz*/
	    /*Creo el círculo*/
	    svg= "<circle cx=\""+(lugar)
		+"\" cy=\""+nivel*150+"\" r=\""+(radio)+"\" stroke=\"black\" stroke-width=\"3\" fill =\""+(color)+"\" />\n";
	    /*Creo el texto*/
	    svg= svg.concat("<text fill=\""+(colorTexto)+"\""
			    +" font-family=\"sens-serif\" font-size=\"15\""
			    +" x=\""+(lugar)+"\" y=\""+((nivel*150)+5)+"\""
			    +" text-anchor=\"middle\">"+vertice.elemento+"</text>");	
	    /*Caso donde tiene hijo derecho, creo linea y recurso sobre hijo derecho*/
	    if(vertice.hayDerecho()){
		svg = svg.concat("<line x1=\""+(lugar)+
				 "\" y1=\""+((nivel*150)+20)+"\" x2=\""+(lugar+div)+
				 "\" y2=\""+((nivel+1)*150)+"\" stroke=\"black\" stroke-width=\"3\" />\n");
		svg= svg.concat(creaVertice(vertice.derecho, nivel+1, ((div)+lugar), div/2));
	    }
	    /*Caso donde tiene hijo izquierdo, creo linea y recurso sobre hijo izquierdo*/
